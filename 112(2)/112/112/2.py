...
class HumanoidBadmintonGame:
    def __init__(self):
        ...
        # 历史轨迹队列（最多保留40个点）
        self.shuttle_hist = deque(maxlen=40)
        # 轨迹更新计数器：每隔多少帧更新一次
        self.trail_update_counter = 0
        self.trail_update_interval = 128  # 大约每0.03秒更新一次
        ...
        print("初始化完成：改进版人形机器人，含实际轨迹绘制（性能优化）。")
    ...
    def _add_court_lines(self):
        ...
        add_box([0, 0, z / 2.0], [half_len + 0.15, half_doubles + 0.15, z / 2.0], court_green)
        add_box(
        ...
        add_box([0, 0, z + 0.003], [line_w / 2.0, half_doubles, line_h], [0.8, 0.8, 0.8, 0.45])
    ...
    def _create_shuttle(self):
        ...
        head_vis = p.createVisualShape(p.GEOM_SPHERE, radius=head_r, rgbaColor=[0.8, 0.5, 0.2, 1.0])
        shuttle = p.createMultiBody(0.004, feather_col, feather_vis, [0, 0, 1.2])
        head_body = p.createMultiBody(
            0.001, head_col, head_vis,
            [0, 0, 1.2 + feather_h / 2 + head_r],
        )
        p.createConstraint(
            shuttle, -1, head_body, -1, p.JOINT_FIXED,
    ...
    def _ensure_urdf_exists(self):
        ...
        self._create_urdf("models/humanoid_left.urdf", "humanoid_left",
                          "0.2 0.6 0.9 1.0", "0.05 0.15 0.35 1.0", "left")
        self._create_urdf("models/humanoid_right.urdf", "humanoid_right",
                          "0.85 0.15 0.15 1.0", "0.35 0.05 0.05 1.0", "right")
    ...
    def _create_urdf(self, path, robot_name, cloth_rgba, dark_rgba, racket_side):
        ...
        left_racket_xml = self._arm_xml("left", "cloth", "skin", "glove", "racket_mat", racket_side == "left")
        right_racket_xml = self._arm_xml("right", "cloth", "skin", "glove", "racket_mat", racket_side == "right")
...
'''
    ...
    def _set_neutral_pose(self):
        neutral = {
            ...
            "left_shoulder_pitch": 0.3, "left_elbow": 0.3, "left_wrist": 0.0,
            "right_shoulder_pitch": 0.3, "right_elbow": 0.3, "right_wrist": 0.0,
        }
    ...
    # 轨迹绘制（低频更新，自动清理）
    # ------------------------------------------------------------------
    def draw_actual_trail(self):
        history = np.array(list(self.shuttle_hist))
        if len(history) < 2:
            return
        ...
        for robot, walk in [(self.left_robot, self.walk_left), (self.right_robot, self.walk_right)]:
            for side in ["left", "right"]:
                ...
                robot.set_joint_positions(
                    {f"{side}_hip": hip, f"{side}_knee": knee, f"{side}_ankle": ankle},
                    velocity=3, force=120,
                )
    ...
    def check_and_hit(self, racket_pos, racket_vel, shuttle_pos, shuttle_vel, robot_side):
        ...
        if dist < 0.75 and good_height and enough_cooldown and swinging_forward:
            ...
            print(f"{robot_side.upper()} RACKET HIT: dist={dist:.2f}, racket_vel={racket_vel.round(2)}, target_vel={target_vel.round(2)}")
            return force
        return None
    ...
    def update_swing_animation(self, dt):
        if self.swing_left:
            self.swing_timer_left = self._update_one_swing(
                robot=self.left_robot, side="left", timer=self.swing_timer_left, dt=dt)
            if self.swing_timer_left < 0:
                self.swing_left = False
                self.swing_timer_left = 0.0
        if self.swing_right:
            self.swing_timer_right = self._update_one_swing(
                robot=self.right_robot, side="right", timer=self.swing_timer_right, dt=dt)
    ...
    def _update_one_swing(self, robot, side, timer, dt):
        ...
        if timer < 0.16:
            ...
            robot.set_joint_positions(
                {f"{side}_shoulder_pitch": 0.25, f"{side}_elbow": 0.35, f"{side}_wrist": 0.0},
                velocity=4, force=80,
            )
            return -1.0
        robot.set_joint_positions(
        ...
        try:
            while True:
                ...
                left_racket, self.left_racket_vel = self.get_racket_position_and_velocity(
                    self.left_robot.robot_id, self.left_racket_link,
                    self.prev_left_racket_pos, dt,
                )
                right_racket, self.right_racket_vel = self.get_racket_position_and_velocity(
                    self.right_robot.robot_id, self.right_racket_link,
                    self.prev_right_racket_pos, dt,
                )
                ...
                force = None
                if self.shuttle_pos[0] < 0:
                    force = self.check_and_hit(left_racket, self.left_racket_vel,
                                               self.shuttle_pos, self.shuttle_vel, "left")
                else:
                    force = self.check_and_hit(right_racket, self.right_racket_vel,
                                               self.shuttle_pos, self.shuttle_vel, "right")

                self.shuttle_pos, self.shuttle_vel = self.shuttle_phys.update(
                    self.shuttle_pos, self.shuttle_vel, dt, force,
                )
